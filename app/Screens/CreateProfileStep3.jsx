...
export default function CreateProfileStep3({ navigation,route }) {
  ...
  const saveCityToFirebase = async () => {
    if (!city) {
  ...
  const extractInformation = (text) => {
    try {
      ...
      // Extract city (any noun)
      //const cityMatch = doc.nouns().first().text(); 
      const cityMatch = doc.match('#City').text() || doc.nouns().first().text();
  
      // // Extract language using specific keywords
      // const languageMatch = doc
      //   .match('(language|tongue|speak)')
      //   .terms()
      //   .last()
      //   .text();
  
      // // Extract profession using noun patterns or specific terms
      // const professionMatch = doc
      //   .match('(am|is|are) [a-z]+') // Matches sentences like "I am a driver"
      //   .terms()
      //   .last()
      //   .text();
  
      // // Extract years of experience using number patterns
      // const experienceMatch = doc
      //   .match('#Value+ (year|years|experience)')
      //   .text();
  
      // Autofill based on matches
      if (cityMatch) {
        setCity(cityMatch);
      }
      // if (languageMatch) {
      //   Alert.alert('Extracted Language', languageMatch);
      // }
      // if (professionMatch) {
      //   Alert.alert('Extracted Profession', professionMatch);
      // }
      // if (experienceMatch) {
      //   Alert.alert('Extracted Experience', experienceMatch);
      //   setCity(cityMatch);
      // }
  ...
  return (
    <View style={styles.container}>
      {/* <Text style={styles.header}>
        Creating Your Profile <Text style={styles.stepNumber}>(3/6)</Text>
      </Text> */}

      <Text style={styles.header}>
        {t('creatingProfile')} <Text style={styles.stepNumber}>{t('step', { step: 3 })}</Text>
      </Text>
      ...
      <View style={styles.inputContainer}>
        ...
        <Text style={styles.question}>{t('cityQuestion')}</Text>
        <TextInput
          style={styles.input}
      ...
      <TouchableOpacity
        ...
        // onPress={() => navigation.navigate('CreateProfileStep4', { language: 'English' })}
        onPress={saveCityToFirebase} 
      >
